...
if NOT "%PM_CMAKE_PATH%" == "" (
    if exist "%PM_CMAKE_PATH%/bin/cmake.exe" (
        GOTO CMAKE_EXTERNAL
    )
)
...
IF NOT "%PM_PYTHON%" == "" GOTO PYTHON_READY

IF NOT "%PM_python_PATH%" == "" (
    IF EXIST "%PM_python_PATH%\python.exe" (
...
for /f "usebackq tokens=*" %%i in (`"%PM_vswhere_PATH%\VsWhere.exe" -version [17.0,19.0^) -latest -products * -requires Microsoft.VisualStudio.Component.VC.Tools.x86.x64 -property installationPath`) do (

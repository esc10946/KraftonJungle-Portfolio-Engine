...
:: 1. PhysX 준비 및 프로젝트 파일 생성
echo.
echo [1/4] Preparing dependencies and project files...
call "%SOLUTION_DIR%GenerateProjectFiles.bat" --no-pause
if %ERRORLEVEL% neq 0 (
...
:: 2. MSBuild로 Release x64 빌드
echo.
echo [2/4] Building Release x64...
...
if not exist "%BUILD_OUTPUT%\KraftonEngine.exe" (
    echo Missing build output: "%BUILD_OUTPUT%\KraftonEngine.exe"
    pause
    exit /b 1
)

:: 3. 기존 ReleaseBuild 폴더 정리
echo.
...
if exist "%PROJECT_DIR%\Data\" (
    ...
    echo Optional Data directory not found; skipping.
)

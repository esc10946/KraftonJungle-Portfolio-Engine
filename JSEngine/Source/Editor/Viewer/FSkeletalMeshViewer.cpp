...
FSkeletalMeshViewer::~FSkeletalMeshViewer()
{
    Shutdown();
}

void FSkeletalMeshViewer::Init(FWindowsWindow* InWindow, UEditorEngine* InEditor, UWorld* InWorld, FSelectionManager* InSelectionManager)
{
...
void FSkeletalMeshViewer::Shutdown()
{
...
void FSkeletalMeshViewer::SelectSocket(int32 SocketIndex)
{
    if (!ViewTarget)
    {
        return;
...
void FSkeletalMeshViewer::ClearSelection()
{
    SelectedBoneIndex = -1;
    SelectedSocketIndex = -1;
...
void FSkeletalMeshViewer::NotifySocketDeleted(int32 SocketIndex)
{
...
bool FSkeletalMeshViewer::HandleBonePick(float LocalX, float LocalY)
{
...
bool FSkeletalMeshViewer::TryPickBone(float LocalX, float LocalY, int32& OutBoneIndex) const
{
    OutBoneIndex = -1;
...
void FSkeletalMeshViewer::SetSocketPreviewMesh(const FName& SocketName, const FString& StaticMeshPath)
{
    if (!ViewTarget)
        return;
...
void FSkeletalMeshViewer::ClearSocketPreview(const FName& SocketName)
{
...
void FSkeletalMeshViewer::ClearAllSocketPreviews()
{
    if (ViewTarget)
    {
...
UStaticMeshComponent* FSkeletalMeshViewer::FindPreviewMesh(const FName& SocketName) const
{
    auto It = SocketPreviewMeshes.find(SocketName);
    return It != SocketPreviewMeshes.end() ? It->second : nullptr;
}

void FSkeletalMeshViewer::ChangeTarget(const FString& InFileName)
{
    FileName = InFileName;

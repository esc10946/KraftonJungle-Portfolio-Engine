...
namespace
{
	...
	int32 CollectChildBoneLocalOffsets(const FSkeletonAsset* SkeletonAsset, int32 BoneIndex, TArray<FVector>& OutChildLocalOffsets)
	{
	...
	bool TryGetBodyAxisAndLength(const FSkeletonAsset* SkeletonAsset, int32 BoneIndex, FVector& OutAxis, float& OutLength)
	{
		...
		{
			const FVector ParentOriginInBoneLocal = Bone.LocalMatrix.GetInverse().TransformPositionWithW(FVector::ZeroVector);
	...
	void AddShapeToBody(UPhysicsBodySetup* BodySetup, const FName& BoneName, const FVector& LocalAxis, float BoneLength, const FPhysicsAssetCreateParams& CreateParams)
	{
		if (!BodySetup)
		{
			return;
		...
		case EPhysicsAssetPrimitiveType::Box:
		{
			...
			BoxShape.LocalTransform.SetRotation(
				CreateParams.bAutoOrientToBone
		...
		default:
		{
			...
			CapsuleShape.LocalTransform.SetRotation(
				CreateParams.bAutoOrientToBone
		...
		const FPhysicsAssetCreateParams& CreateParams)
	{
		...
		for (const FVertexPNCTBW& Vertex : MeshAsset->Vertices)
		{
			if (CreateParams.VertexWeightingType == EPhysicsAssetVertexWeightingType::DominantWeight)
			{
				...
				{
					const FVector LocalPosition = SkeletonAsset->Bones[BoneIndex].InverseBindPoseMatrix.TransformPositionWithW(Vertex.Position);
		...
		int32 TargetBoneIndex)
	{
		...
		if (!IsValidBoneIndex(SkeletonAsset, SourceBoneIndex) ||
			!IsValidBoneIndex(SkeletonAsset, TargetBoneIndex) ||
		...
		const FMatrix SourceToTarget =
			SkeletonAsset->Bones[SourceBoneIndex].GlobalMatrix *
	...
	FMergedBoneFitData BuildMergedBoneFitData(
		const FSkeletonAsset* SkeletonAsset,
		...
		const FPhysicsAssetCreateParams& CreateParams)
	{
		...
		for (int32 BoneIndex = BoneCount - 1; BoneIndex >= 0; --BoneIndex)
		{
			...
			{
				AppendFitToBoneSpace(SkeletonAsset, Data.ExtraFits[ParentIndex], BoneFits[BoneIndex], BoneIndex, ParentIndex);
			}
			AppendFitToBoneSpace(SkeletonAsset, Data.ExtraFits[ParentIndex], Data.ExtraFits[BoneIndex], BoneIndex, ParentIndex);
		}
	...
	void AddFittedShapeToBody(UPhysicsBodySetup* BodySetup, const FName& BoneName, const FBoneShapeFit& Fit, const FPhysicsAssetCreateParams& CreateParams)
	{
		...
		case EPhysicsAssetPrimitiveType::Sphere:
		{
			...
			SphereShape.Radius = (std::max)((std::max)(BoxExtent.X, (std::max)(BoxExtent.Y, BoxExtent.Z)) * CreateParams.FitPadding, CreateParams.MinPrimitiveSize);
			SphereShape.CollisionDesc = CollisionDesc;
			ShapeSetup.SphereShapeSetups.push_back(SphereShape);
			break;
		}
		...
		default:
		{
			...
			{
				CapsuleShape.Radius = (std::max)((std::max)(BoxExtent.Y, BoxExtent.Z) * CreateParams.FitPadding, CreateParams.MinPrimitiveSize);
				CapsuleShape.Length = (std::max)(BoxExtent.X * 2.0f * CreateParams.FitPadding - CapsuleShape.Radius * 2.0f, 0.0f);
			}
			else if (BoxExtent.Y > BoxExtent.X && BoxExtent.Y > BoxExtent.Z)
			{
				...
				CapsuleShape.Radius = (std::max)((std::max)(BoxExtent.X, BoxExtent.Z) * CreateParams.FitPadding, CreateParams.MinPrimitiveSize);
				CapsuleShape.Length = (std::max)(BoxExtent.Y * 2.0f * CreateParams.FitPadding - CapsuleShape.Radius * 2.0f, 0.0f);
			}
			else
			{
				AxisRotation = MakeAxisAlignmentQuat(PhysicsAssetCapsuleAxis, FVector::ZAxisVector);
				CapsuleShape.Radius = (std::max)((std::max)(BoxExtent.X, BoxExtent.Y) * CreateParams.FitPadding, CreateParams.MinPrimitiveSize);
				CapsuleShape.Length = (std::max)(BoxExtent.Z * 2.0f * CreateParams.FitPadding - CapsuleShape.Radius * 2.0f, 0.0f);
			}
		...
		const FBoneShapeFit* Fit)
	{
		...
		const FBone& Bone = SkeletonAsset->Bones[BoneIndex];
		return (bHasBoneSize && BoneLength >= CreateParams.MinBoneSize) ||
			(Bone.ParentIndex == -1 && bHasBoneSize);
	}

	bool PopulateVertexWeightPhysicsBodies(
	...
	void PopulateBoneLengthPhysicsBodies(
		UPhysicsAsset* PhysicsAsset,
		const FSkeletonAsset* SkeletonAsset,
		const FPhysicsAssetCreateParams& CreateParams,
		TMap<int32, UPhysicsBodySetup*>& OutBodyByBoneIndex)
	...
	void BuildGeneratedPhysicsBodies(
		UPhysicsAsset* PhysicsAsset,
		const FSkeletalMesh* MeshAsset,
		const FSkeletonAsset* SkeletonAsset,
		const FPhysicsAssetCreateParams& CreateParams,
		TMap<int32, UPhysicsBodySetup*>& OutBodyByBoneIndex)
	{
		...
		{
			bool bPopulated = PopulateVertexWeightPhysicsBodies(PhysicsAsset, MeshAsset, SkeletonAsset, CreateParams, OutBodyByBoneIndex);
			if (!bPopulated)
			{
				FPhysicsAssetCreateParams RetryParams = CreateParams;
				RetryParams.MinBoneSize = 1.0f;
				bPopulated = PopulateVertexWeightPhysicsBodies(PhysicsAsset, MeshAsset, SkeletonAsset, RetryParams, OutBodyByBoneIndex);
				if (bPopulated && GLastPhysicsAssetCreateWarning.empty())
				{
					GLastPhysicsAssetCreateWarning = "Vertex-weight PhysicsAsset generation needed a relaxed MinBoneSize retry.";
				}
			}
			...
			if (!CreateParams.bFallbackToBoneLength)
			{
				...
				{
					GLastPhysicsAssetCreateWarning = "Vertex-weight PhysicsAsset generation failed and bone-length fallback is disabled.";
				}
				return;
			}

			GLastPhysicsAssetCreateWarning = "Vertex-weight PhysicsAsset generation could not create any body. Falling back to bone-length generation.";
		}

		PopulateBoneLengthPhysicsBodies(PhysicsAsset, SkeletonAsset, CreateParams, OutBodyByBoneIndex);
	}

	void BuildGeneratedPhysicsConstraints(
		UPhysicsAsset* PhysicsAsset,
		const FSkeletonAsset* SkeletonAsset,
		const TMap<int32, UPhysicsBodySetup*>& BodyByBoneIndex,
		const FPhysicsAssetCreateParams& CreateParams)
	{
		...
		for (const auto& Pair : BodyByBoneIndex)
		{
			...
			const int32 ParentBodyBoneIndex = FindParentBodyBoneIndex(SkeletonAsset, BodyByBoneIndex, BoneIndex, CreateParams.bWalkPastSmallBones);
			...
			ConstraintSetup.AngularLimit = CreateParams.AngularConstraintMode == EPhysicsAssetAngularConstraintMode::Locked ? 0.0f : 45.0f;
			ConstraintSetup.TwistLimitMin = CreateParams.AngularConstraintMode == EPhysicsAssetAngularConstraintMode::Free ? -180.0f : -45.0f;
			ConstraintSetup.TwistLimitMax = CreateParams.AngularConstraintMode == EPhysicsAssetAngularConstraintMode::Free ? 180.0f : 45.0f;
			ConstraintSetup.SwingLimitY = CreateParams.AngularConstraintMode == EPhysicsAssetAngularConstraintMode::Free ? 180.0f : 30.0f;
			ConstraintSetup.SwingLimitZ = CreateParams.AngularConstraintMode == EPhysicsAssetAngularConstraintMode::Free ? 180.0f : 30.0f;
	...
	ID3D11Device* Device)
{
	...
	BuildGeneratedPhysicsBodies(PhysicsAsset, SkeletalMesh->GetSkeletalMeshAsset(), SkeletonAsset, CreateParams, BodyByBoneIndex);

...
bool UParticleSystem::Save(const FString& Path)
{
    ...
    FAssetImportMetadata Metadata; // 빈 메타데이터
    Writer << Metadata;
...
UParticleSystem* UParticleSystem::Load(const FString& Path)
{
    ...
    UParticleSystem* System = GUObjectArray.CreateObject<UParticleSystem>();
    System->Serialize(Reader);

    if (!Reader.IsValid())
    {
        GUObjectArray.DestroyObject(System);
        return nullptr;
    }

    System->AssetPath = Path;
    return System;
}

...
namespace
{
	bool HasProjectContentRoot(const std::filesystem::path& Candidate)
	{
		std::error_code Error;
		const std::filesystem::path ContentDir = Candidate / L"Content";
		return std::filesystem::exists(ContentDir, Error) && std::filesystem::is_directory(ContentDir, Error);
	}

	std::filesystem::path FindProjectRootAbove(std::filesystem::path Start)
	{
		Start = Start.lexically_normal();
		for (;;)
		{
			if (HasProjectContentRoot(Start))
			{
				return Start;
			}
...
std::wstring FPaths::RootDir()
{
	...
	if (Cached.empty())
	{
		...
		// 배포 빌드는 exe 옆에 Content가 있고, 개발 실행은 보통 CWD가 프로젝트 루트다.
		// 둘 다 아니면 Bin/Game 같은 실행 폴더에서 부모를 타고 올라가 프로젝트 루트를 찾는다.
		if (HasProjectContentRoot(ExeDir))
		{
			RootPath = ExeDir;
		}
		else
		{
			...
			if (!Error && HasProjectContentRoot(CurrentDir))
			{
				RootPath = CurrentDir;
			}
			else if (std::filesystem::path Found = FindProjectRootAbove(ExeDir); !Found.empty())
			{
				RootPath = Found;
			}
			else if (!Error)
			{
				if (std::filesystem::path Found = FindProjectRootAbove(CurrentDir); !Found.empty())
				{
					RootPath = Found;
				}
			}
		...
		{
			std::error_code Error;
			RootPath = std::filesystem::current_path(Error);
			if (Error)
			{
				RootPath = ExeDir;
			}
